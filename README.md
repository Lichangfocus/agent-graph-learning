...
**方式一（Claude Code）**：克隆本仓库，在仓库目录里打开 Claude Code，直接说：

```
我想学 光合作用，给 8 岁孩子
```

skill 会自动触发：对焦提问 → 生成图谱 → 结构校验 → 写图文卡 → 组装出 `maps/<主题>/index.html`，浏览器打开即玩。

**方式二（任何 agent 环境）**：把 [prompts/workbuddy.md](prompts/workbuddy.md) 的内容整段发给你的 agent。
